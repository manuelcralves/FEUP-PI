...
import { LinhaDocumentoInterno } from 'src/app/shared/interfaces/linha-documento-interno';
...
export class ExpenseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.EditorDocumento = this.formBuilder.group({
      ...
      Fornecedor: [undefined, Validators.maxLength(12)],
  ...
  onSubmit(form: FormGroup) {
   ...
   this.gravaRascunho(form, "E");    
    ...
    loadSketch() {
        ...
        this.internosService.getRascunho(this.sketchId).subscribe({
            next: (documento: DocumentoInterno) => {
    ...
    loadApprovalData(documento: DocumentoInterno){
    ...
    loadRejectData(documento: DocumentoInterno){
...
gravaRascunho(form: any, estado: string){
    ...
    let documento: DocumentoInterno = { 
        ...
        Utilizador: this.documentoControls.Utilizador.value,
    ...
    this.blockUI.start('A gravar rascunho...');

    if (this.sketchId) {
        this.internosService.alterarRascunho(this.sketchId, documento, this.dropZoneFiles).subscribe({
            next: res => {
                this.blockUI.stop();
            ...
            error: (error: any) => {
                ...
                this.toastr.error("Não foi possível ligar ao servidor ou não foi possivel enviar alguns dos anexos selecionados. Confirme se tem acesso aos ficheiros que selecionou.", "Erro a gravar rascunho", { timeOut: 9000 });
                else
                this.apiErrorService.handleError(error, "Erro a gravar rascunho");
            }
        });
    }
    else {
        this.internosService.criarRascunho(documento, this.dropZoneFiles).subscribe({
            ...
            error: (error: any) => {
                ...
                if (this.dropZoneFiles.length > 0 && typeof error == "object" && error instanceof HttpErrorResponse && error.status === 0)
                this.toastr.error("Não foi possível ligar ao servidor ou não foi possivel enviar alguns dos anexos selecionados. Confirme se tem acesso aos ficheiros que selecionou.", "Erro a criar rascunho", { timeOut: 9000 });
                else
                this.apiErrorService.handleError(error, "Erro a criar rascunho");
            }
        });
    }
}
...
gravarDocumento(form: any) {
 ...
 if(this.obraId){
  ...
  if (this.documentoId) {
      this.internosService.alterarDocumento(this.documentoId, documento, this.dropZoneFiles).subscribe({
          next: res => {
              this.blockUI.stop();
      ...
      this.internosService.criarDocumento(documento, this.dropZoneFiles).subscribe({
          next: (documentoId: string) => {
              this.blockUI.stop();
...
resetVars() {
  ...
  loadTiposDocumento() {
    ...
    this.internosService.getTiposDocumento().subscribe({
...
loadSeries(tipodoc: string) {
  ...
  this.pedidoSeries = new Promise<void>((resolve, reject) => {
      this.internosService.getSeries(tipodoc).subscribe({
...
getProximoNumeroSerie() {
  ...
  this.internosService.getProximoNumeroSerie(this.documentoControls.TipoDoc.value, this.documentoControls.Serie.value).subscribe({
...
LstObrasModal() {
  ...
  adicionaLinha(data){
    if (this.linhaEditar) {
        ...
        this.linhaEditar.DataEntrega = new Date( data.DataEntrega.year, data.DataEntrega.month-1, data.DataEntrega.day);
        ...
        this.lstLinhas.push({
            ...
            DataEntrega: new Date( data.DataEntrega.year, data.DataEntrega.month -1, data.DataEntrega.day),
    ...
    modalRef.result
        .then((value: any) => {
          ...
          this.blockUI.start('A eliminar documento...');
            ...
            this.internosService.deleteRascunho(this.sketchId).subscribe({
              next: (res: void) => {
                  this.blockUI.stop();
